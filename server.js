  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS users (
        ...
        case_free BIGINT DEFAULT NULL,
        ban INTEGER DEFAULT NULL
      )
    `);
...
app.post('/api/user/:id/free-case', async (req, res) => {
  const userId = req.params.id;
  const { cardKey } = req.body; // Получаем ключ выпавшей карточки

  const COOLDOWN = 21600 * 1000; // 6 часов
  ...
  try {
    ...
    let newCards = (!cards || cards.trim() === '' || cards === '[]') ? 
      cardKey : 
      cards.trim() + ' ' + cardKey;
...
// Получить список всех активных комнат (open rooms list)
app.get('/api/game/rooms', async (req, res) => {
  logRequest(req, '📋 Получение списка активных комнат');
  let client;
  try {
    client = await pool.connect();
    // Получаем только активные комнаты (например, в статусе 'waiting' или 'game')
    const result = await client.query(
      `SELECT id, room_code, player1, player2, game_state, bet, created_at
       FROM game
       WHERE game_state IN ('waiting', 'game')
...
app.post('/api/promocode/check', async (req, res) => {
  ...
  logRequest(req, `🎫 Проверка промокода: ${code} (ID: ${userId})`);

  if (!userId || !code) {
    return res.status(400).json({ 
      status: 'error', 
      message: 'ID пользователя и промокод обязательны' 
    });
  }
  ...
  try {
    ...
    // Ищем промокод (без приведения к верхнему регистру)
    const promocodeResult = await client.query('SELECT * FROM promocodes WHERE code = $1', [code.trim()]);
    ...
    // Проверяем количество оставшихся активаций
    if (promocode.act <= 0) {
      console.log(`❌ Лимит активаций исчерпан: ${code}`);
      return res.json({ 
        status: 'success', 
        valid: false,
    ...
    const activatedUsers = promocode.activated_users ? promocode.activated_users.split(' ') : [];
    if (activatedUsers.includes(userId.toString())) {
      console.log(`❌ Пользователь уже активировал промокод: ${code}`);
      return res.json({ 
        status: 'success', 
        valid: false,
    ...
    console.log(`✅ Промокод действителен: ${code}`);
    res.json({ 
      status: 'success', 
      valid: true,
      promocode: {
    ...
    console.error(`❌ Ошибка проверки промокода ${code}:`, err.message);
    res.status(500).json({ 
      status: 'error', 
...
app.post('/api/promocode/activate', async (req, res) => {
  const { userId, code } = req.body;
  logRequest(req, `🎫 Активация промокода: ${code} (ID: ${userId})`);
  ...
  try {
    ...
    // Получаем промокод (без приведения к верхнему регистру)
    const promocodeResult = await client.query('SELECT * FROM promocodes WHERE code = $1 FOR UPDATE', [code.trim()]);
    ...
    const newBalance = user.balance + (promocode.balance || 0);
    await client.query(
      'UPDATE users SET balance = $1 WHERE id = $2',
      [newBalance, userId]
    );

    // Обновляем промокод (добавляем пользователя в активировавших и уменьшаем счетчик активаций)
    const newActivatedUsers = promocode.activated_users ? 
      `${promocode.activated_users} ${userId}` : 
      userId.toString();

    await client.query(
      'UPDATE promocodes SET activated_users = $1, act = act - 1 WHERE code = $2',
      [newActivatedUsers, promocode.code]
    );

    await client.query('COMMIT');

    console.log(`✅ Промокод активирован: ${code} пользователем ${userId}`);
    res.json({ 
      status: 'success', 
    ...
    await client.query('ROLLBACK');
    console.error(`❌ Ошибка активации промокода ${code}:`, err.message);
    res.status(500).json({ 
      status: 'error', 
...
app.get('/api/statistics', async (req, res) => {
  logRequest(req, '📊 Получение общей статистики');

  let client;
  try {
    client = await pool.connect();

    // Получаем все данные за один запрос (без total_wins)
    const result = await client.query(`
      SELECT 
        (SELECT COUNT(*) FROM users) as total_players,
        (SELECT COALESCE(SUM(balance), 0) FROM users) as total_sticks,
        (SELECT COALESCE(SUM(array_length(string_to_array(TRIM(cards), ' '), 1)), 0) 
         FROM users WHERE cards IS NOT NULL AND cards <> '' AND cards <> '[]') as total_cards,
        (SELECT COALESCE(SUM(games), 0) FROM users) as total_games,
        (SELECT COUNT(*) FROM users WHERE updated_at > NOW() - INTERVAL '5 minutes') as online_count
    `);
    ...
    res.json({ 
      ...
      totalSticks: stats.total_sticks,
